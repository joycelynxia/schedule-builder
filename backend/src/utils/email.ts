...
export interface CoverSwapApprovedData {
  userName: string;
  userEmail: string;
  type: "cover" | "swap";
...
export const sendCoverSwapApprovedEmail = async (data: CoverSwapApprovedData) => {
  if (!process.env.RESEND_API_KEY) {
    console.warn("RESEND_API_KEY not set, skipping cover/swap approved email");
    console.warn("Would send email to:", data.userEmail);
    return;
  }
  ...
  const action = isCover ? "cover" : "take";

  let heading: string;
  let intro: string;
  if (data.role === "requester") {
    heading = `Your ${isCover ? "Cover" : "Swap"} Request Approved`;
    intro = `Your ${isCover ? "cover" : "swap"} request has been approved. ${data.otherPartyName} will ${action} your shift on ${formattedDate} (${formattedStart} – ${formattedEnd}).`;
  } else {
    heading = `Your Request to ${isCover ? "Cover" : "Take"} a Shift Approved`;
    intro = `Your request to ${action} ${data.otherPartyName}'s shift has been approved. The shift on ${formattedDate} (${formattedStart} – ${formattedEnd}) is now assigned to you.`;
  }
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .container { background-color: #ffffff; border: 1px solid #e0e0e0; border-radius: 8px; padding: 30px; }
          .header { color: #7c5cf0; font-size: 24px; font-weight: 700; margin-bottom: 20px; }
          .shift-details { background-color: #f8f9fa; border-left: 4px solid #7c5cf0; padding: 20px; margin: 20px 0; border-radius: 4px; }
      ...
      <body>
        <div class="container">
          ...
          <div class="shift-details">
            ...
            <div class="detail-row"><span class="detail-label">Time:</span><span class="detail-value">${formattedStart} – ${formattedEnd}</span></div>
            ${data.note ? `<div class="detail-row"><span class="detail-label">Note:</span><span class="detail-value">${data.note}</span></div>` : ""}
          </div>
          <p>Please check your schedule for more details.</p>
          <div class="footer"><p>This is an automated notification from Schedulr.</p></div>
        </div>
      </body>
    </html>
  `;

  const text = `Hi ${data.userName},\n\n${intro}\n\nDate: ${formattedDate}\nTime: ${formattedStart} – ${formattedEnd}${data.note ? `\nNote: ${data.note}` : ""}\n\nPlease check your schedule for more details.\n\nThis is an automated notification from Schedulr.`;

  try {
    const result = await resend.emails.send({
      from: fromEmail,
      to: data.userEmail,
      subject: `${heading} - ${formattedDate}`,
      html,
      text,
    });
    console.log(`Cover/swap approved email sent to ${data.userEmail}`, result);
  } catch (error) {
    console.error(`Failed to send cover/swap approved email to ${data.userEmail}:`, error);
